export const englishContent = {
    foodMenu: 'Food Menu',
    options: 'Options:',
    glutenFree: 'Gluten Free',
    foodWarning: '*Consuming raw or undercooked meats, poultry, seafood, shellfish, or eggs may increase your risk of foodborne illness.'
}

export const spanishContent = {
    foodMenu: 'Menú de Comida',
    options: 'Opciones:',
    glutenFree: 'Sin Gluten',
    foodWarning: '*Consumir carnes, aves, mariscos, mariscos o huevos crudos o poco cocidos puede aumentar el riesgo de contraer enfermedades transmitidas por los alimentos.'
}


export const menuEnglishData = {
    Appetizers: [
        {
            name: "Guacamole",
            price: "$10.99",
            description: `A generous serving of fresh homemade guacamole with fresh cilantro, tomato, onion, and jalapeños. Served with a warm corn tortilla chip.
            `,
            options: ["No Spicy - ", "Med Spicy - ", "Spicy"]
        },
        {
            name: "Nachos Sin Fronteras",
            price: "$10.99",
            description: `Corn tortilla chips piled high and topped off with refried beans, pico de gallo, lettuce, fresh jalapeños, sour cream guacamole sauce, and your choice of beef chorizo or chicken.
            `,
            options: ["Chicken - ", "Beef - ", "Chorizo - ", "Pork Carnitas"]
        },
        {
            name: "Quesadilla",
            price: "$11.99",
            description: `1 Large flour tortilla stuffed with cheese and your choice of chicken, chorizo, carnitas, or beef.
            `,
            options: ["Chicken - ", "Beef - ", "Chorizo - ", "Pork Carnitas"]
        },
        {
            name: "Chile con Queso",
            price: "$10.99",
            description: `Our special blend of chile and cheese chorizo. Served with warm corn tortilla chips.
            `,
            options: []
        },
        {
            name: "Yuca con Chicharron",
            price: "$11.99",
            description: `Fried or steamed yucca with fried chicharron (fried pork). Served with fresh cabbage and topped with our sweet cream sauce.
        `,
            options: ["Fried - ", "Steamed"]
        },
        {
            name: "Camarones al Ajillo (Garlic Shrimp)",
            price: "$11.99",
            description: `Sautéed with El Maestro Tequila, garlic, fresh lime, and served with baguette points.
        `,
            options: []
        },
        {
            name: "Fried Calamari",
            price: "$11.99",
            description: `Lightly breaded calamari, available grilled or fried, and served with our famous tomatillo sauce and lime.
        `,
            options: ["Grilled - ", "Fried"]
        },
        {
            name: `3 Soft Tacos`,
            price: "$11.99",
            description: `Soft corn tortilla filled with your choice of chicken, beef, carnitas, chorizo, shrimp, or beef Barbacoa, served with pico de gallo.
        `,
            options: ["Beef - ", "Chicken - ", "Chorizo - ", "Pork Carnitas"]
        },
        {
            name: "Salsa & Chips",
            price: "$1.99",
            description: `Corn tortilla chips with a homemade salsa
        `,
            options: []
        },
        {
            name: "16 oz. Chicken Tortilla Soup",
            price: "$8.99",
            description: `Topped With sour cream, cheese, avocado, cilantro, and tortilla strips
        `,
            options: []
        },
        {
            // gluten free
            name: "2 Chicken Colombian Empanadas",
            price: "$8.99",
            description: `Served with Aji Sauce.`,
            options: []
        },
    ],
    Salads: [
        {
            name: `Acapulco Salad`,
            price: "$19.99",
            description: `Grilled and blackened filet of salmon, paired up with baby greens, tomatoes, cucumber, Monterey cheese, and avocado. Tossed with homemade raspberry chipotle vinaigrette dressing.
        `,
            options: []
        },
        {
            name: `Ensalada Sin Fronteras`,
            price: "$15.99",
            description: `Fresh garden greens, grilled vegetables, beans, pico de gallo, and avocado with your choice of shrimp, tilapia, chicken or beef.
        `,
            options: ["Chicken Fajita - ", "Beef Fajita - ", "Filet of Tilapia - ", "Shrimp"]
        },
        {
            name: `Taco Salad`,
            price: "$14.99",
            description: `A 12" crispy tortilla shell filled with lettuce, tomato, onion, black beans, fresh grated cheese, and with your choice of chicken, beef, shrimp, or vegetarian. With our homemade dressing, sour cream, and guacamole sauce on the side.
        `,
            options: ["Beef - ", "Chicken - ", "Shrimp - ", "Vegetarian - ", "Pork Carnitas"]
        },
    ],
    Entrees: [
        {
            name: `Baleada (2)`,
            price: "$11.99",
            description: `A Honduran specialty. Two tortillas, stuffed with refried beans, fresh grated cheese, avocado, and imported Honduras cream. Choice of carne asada (steak) or chorizo with egg.`,
            options: ["Beef - ", "Chorizo with Egg"]
        },
        {
            name: `Burrito`,
            price: "$11.99",
            description: `A large flour tortilla stuffed with rice, beans, cheese, chipotle, with your choice of chicken, chorizo, carnitas, beef or vegetarian, and topped with guacamole sauce and sour cream.
            `,
            options: ["Beef - ", "Chorizo - ", "Pork Carnitas - ", "Vegetarian"]
        },
        {
            name: `Chimichanga`,
            price: "$11.99",
            description: `Our burrito fried and your choice of chicken chorizo, carnitas, beef or vegetarian, and topped with sour cream and guacamole sauce.
            `,
            options: ["Beef - ", "Chicken - ", "Pork Carnitas - ", "Chorizo - ", "Vegetarian"]
        },
        {
            name: `Cuban Sandwich`,
            price: "$10.99",
            description: `Miami style pork, ham, Swiss cheese, dill pickles, and mustard. Served with your choice of French fries, fried sweet plantains, yucca or a small salad.
            `,
            options: ["French Fries - ", "Yucca - ", "Sweet Plantains - ", "Small Salad"]
        },
        {
            name: `Pollo Tropical`,
            price: "$16.99",
            description: `Grilled chicken breast marinated with Caribbean spices and topped with tropical curry mango salsa. Made with onions, cilantro, tomatoes, sweet peppers, and served with rice.
            `,
            options: []
        },
        {
            name: `Pollo a La Barbacoa`,
            price: "$17.99",
            description: `A ¼ chicken, Mexican style, wrapped in waxed paper. Marinated with herbs and topped with our famous Mexican barbacoa sauce. Served with a rice tortilla and green secret sauce on the side.
            `,
            options: []
        },
        {
            name: `Pollo Ranchero`,
            price: "$17.99",
            description: `Chicken breast lightly floured and cooked with White Wine . Topped with ranchero sauce and Monterey cheese. Served with rice and salad.
            `,
            options: []
        },
        {
            name: `Enchiladas Mexicanas`,
            price: "$14.99",
            description: `Three soft, hand-rolled tortillas filled with your choice of chicken, chorizo, carnitas beef or vegetarian. Served with casamiento.
            `,
            options: ["Mole Sauce - ", "Tomatillo Sauce - ", "Carnitas - ", "Beef - ", "Chicken - ", "Chorizo - ", "Vegetarian"]
        },
        {
            name: `Chicharron en Salsa Verde`,
            price: "$17.99",
            description: `Crispy sautéed pork with onions, cilantro, and topped with green tomatillo sauce. Served with flour tortillas and rice.
            `,
            options: []
        },
        {
            name: `Chile Relleno`,
            price: "$19.99",
            description: `A toasty poblano pepper filled with cheese, tomatillo sauce and your choice of chicken or vegetarian. Served with casamiento.
            `,
            options: ["Chicken - ", "Vegetarian"]
        },
        {
            name: `Fajitas`,
            price: "$20.99",
            description: `Sautéed onions and peppers, served with casamiento, flour tortillas and your choice of chicken, beef or shrimp. (Additional cost for proteins).
            `,
            options: ["Beef - ", "Chicken - ", "Shrimp - ", "Shrimp Chicken Beef +$2 - ", "Beef and Chicken +$2"]
        },
        {
            name: `Peruvian Lomo Saltado`,
            price: "$19.99",
            description: `A slice of tender beef steak, cooked with olive oil, a soy sauce white wine Aji, pepper, tomato, and onion. Served over French fries or Yucca. Comes with a side of rice.
            `,
            options: ["French Fries - ", "Yuca"]
        },
        {
            name: `Ropa Vieja`,
            price: "$21.99",
            description: `Shredded marinated beef, cooked with red wine and fresh vegetables. Served with casamiento and sweet plantains.
            `,
            options: []
        },
        {
            name: `Chile en Nogada`,
            price: "$19.99",
            description: `A toasty poblano pepper, filled with seasoned ground brisket beef and combined with raisins and sweet plantains. Topped with our delicious, homemade cold creamy Nogada Sauce and served with white rice.
            `,
            options: []
        },
        {
            name: `Camarones Michoacanos`,
            price: "$18.99",
            description: `Sautéed shrimp with green and red peppers, onion, and Mexican chipotle cream sauce. Served with white rice and flour tortillas.
            `,
            options: []
        },
        {
            name: `Carne Asada a La Mexicana`,
            price: "$18.99",
            description: `New York steak marinated and topped with ranchero sauce. Served with a house salad and casamiento.
            `,
            options: ["Casamiento"]
        },
        {
            name: `Pescado Oaxaca`,
            price: "$18.99",
            description: `Grilled fillet of tilapia, topped with white wine chipotle cream sauce, peppers and Serrano ham. Served with house salad and casamiento.
            `,
            options: ["Casamiento"]
        },
        {
            name: `Margarita Salmon`,
            price: "$21.99",
            description: `A fillet of salmon, marinated in tequila, fresh lime juice, spices, and cooked with garlic and tomatoes. Served with chipotle mashed potatoes and grilled asparagus.
            `,
            options: []
        },
        {
            name: `El Maestro Seared Ahi Tuna`,
            price: "$20.99",
            description: `A fillet of tuna encrusted with Peruvian spices on top of garlic chipotle mashed potatoes and served with julienne vegetables sautéed with El Maestro tequila, soy sauce, fresh ginger and a touch of red roasted wasabi sauce.
            `,
            options: ["Meduium - ", "Medium Rare - ", "Rare"]
        },
        {
            name: `Tilapia Caribbean Style`,
            price: "$16.99",
            description: `Marinated fillet of tilapia cooked with white wine, onions, and sweet peppers. Topped with mango chutney sauce and served with rice and a house salad.
            `,
            options: []
        },
        {
            name: `Pork Chop a La Mamajuana`,
            price: "$22.99",
            description: `A thick-cut pork chop marinated in citrus and topped with Serrano ham and onion rings. Served with mashed potatoes, sautéed vegetables and homemade Mamajuana Liquor sauce.
            `,
            options: []
        },
        {
            name: `Picadillo Cubano`,
            price: "$15.99",
            description: `Ground beef, raisins, peppers, onions, and garlic, seasoned Cuban Style. Served with a side of rice and fried sweet Plantains.
            `,
            options: []
        },
    ],
    Desserts: [
        {
            name: `Flan Latino`,
            price: "$5.99",
            description: `With House-Made Caramel Sauce.
            `,
            options: []
        },
        {
            name: `Mexican Banana Tempura`,
            price: "$7.99",
            description: `With Vanilla Ice Cream on the Side.
            `,
            options: []
        },
        {
            name: `Latin Bread Pudding`,
            price: "$7.99",
            description: `With Vanilla Ice Cream on the side.
            `,
            options: []
        },
    ]
};

export const demoEnglishData = {
    Appetizers: [
        {
            name: "Guacamole",
            price: "$10.99",
            description: `A generous serving of fresh homemade guacamole with fresh cilantro, tomato, onion, and jalapeños. Served with a warm corn tortilla chip.
            `,
            options: ["No Spicy - ", "Med Spicy - ", "Spicy"]
        }
    ],
    Salads: [
        {
            name: `Acapulco Salad`,
            price: "$19.99",
            description: `Grilled and blackened filet of salmon, paired up with baby greens, tomatoes, cucumber, Monterey cheese, and avocado. Tossed with homemade raspberry chipotle vinaigrette dressing.
        `,
            options: []
        }
    ],
    Entrees: [
        {
            name: `Baleada (2)`,
            price: "$11.99",
            description: `A Honduran specialty. Two tortillas, stuffed with refried beans, fresh grated cheese, avocado, and imported Honduras cream. Choice of carne asada (steak) or chorizo with egg.`,
            options: ["Beef - ", "Chorizo with Egg"]
        }
    ],
    Desserts: [
        {
            name: `Flan Latino`,
            price: "$5.99",
            description: `With House-Made Caramel Sauce.
            `,
            options: []
        }
    ]
};

export const menuSpanishData = {
    Aperitivos: [
        {
            name: "Guacamole",
            price: "$10.99",
            description: `A generous serving of fresh homemade guacamole with fresh cilantro, tomato, onion, and jalapeños. Served with a warm corn tortilla chip.
            `,
            options: ["No Spicy - ", "Med Spicy - ", "Spicy"]
        }
    ],
    Ensaladas: [
        {
            name: `Acapulco Salad`,
            price: "$19.99",
            description: `Grilled and blackened filet of salmon, paired up with baby greens, tomatoes, cucumber, Monterey cheese, and avocado. Tossed with homemade raspberry chipotle vinaigrette dressing.
        `,
            options: []
        }
    ],
    'Platos Principales': [
        {
            name: `Baleada (2)`,
            price: "$11.99",
            description: `A Honduran specialty. Two tortillas, stuffed with refried beans, fresh grated cheese, avocado, and imported Honduras cream. Choice of carne asada (steak) or chorizo with egg.`,
            options: ["Beef - ", "Chorizo with Egg"]
        }
    ],
    Postres: [
        {
            name: `Flan Latino`,
            price: "$5.99",
            description: `With House-Made Caramel Sauce.
            `,
            options: []
        }
    ]
};
